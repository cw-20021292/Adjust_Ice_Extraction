...
static void IceFrontPercentIncrease(U8 u8IceOut)
{
    ...
    if(IcePosition.u8IceFrontCheck == SET)
    {
        ...
        if(IcePosition.u16BeforeIceOutTime >= 100)    // 10초 이상
        {
            IcePosition.u16BeforeIceOutTime = 0;
            // 누적 10초 이상마다 front percentage 상승
            IcePosition.f32IceFrontPercent += 0.10F;  // 추출시간 10%씩 감소
        }
    }
...
static void IceFrontPercentDecrease(U8 u8IceStirStatus)
{
...
void IcePositionManager(void)
{
    ...
    IceFrontPercentDecrease(u8IceStirStatus);
}
