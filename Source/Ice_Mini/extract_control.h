...
#define ICE_FRONT_DETECT_TIME          40    // 4초
#define ICE_LEAVE_TIME                 (U16)(3 * 60 * 600)    // 3 hour
typedef struct
{
    F32 f32IceFrontPercent;       // (안쏠림) 0 ~ 1.0 (완전 쏠림)
    U16 u16BeforeIceOutTime;      // 이전 총 얼음추출 시간
    U8  u8LeaveCheck;             // 얼음 미추출 방지 체크 (0:미체크, 1:체크)
    U16 u16LeaveTime;             // 얼음 미추출 방지 시간 (3 hour)
    U8  u8IceOutStatus;           // 얼음추출 상태 (0:미추출, 1:추출중)
    U8  u8IceOutStatusOld;        // 이전 얼음추출 상태
    U8  u8IceFrontCheck;          // 얼음위치 상태 (0:미체크, 1:체크)
    U8  u8IceStirStatus;          // 얼음섞기 상태 (0:미섞기, 1:섞기)
} IcePosition_t;
...
#define ICE_EXTRACT_TIME__10_EA        70 // 25 // 너무 짧음
...
#define BUFFER_DRAIN_TIME					10			// ★2025-06-02 Phil
#define BUFFER_DRAIN_TIME_FOR_HIGH_TEMP 	30			// ★U2025-06-11 Phil
