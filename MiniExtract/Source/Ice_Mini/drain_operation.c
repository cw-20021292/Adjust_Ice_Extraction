...
void hot_water_extract_after_check(void);
...
#define            Bit1_Drain_Low_1_Hour                       U16DrainOperateB.Bit.b1      /* 중수위 1시간 이상 유지 시 동작 */
...
#define            Bit10_Drain_Acid_Clean                      U16DrainOperateB.Bit.b10     /* ACID CLEAN (미사용) */
#define            Bit11_Drain_Hot_Extract_After               U16DrainOperateB.Bit.b11     /* [2025-12-19] CH.PARK 온수 추출 후 배수 기능 */
...
U16 gu16_drain_low_water_1hour_tmr;
...
/***********************************************************************************************************************
* Function Name: keep_1_hour_check
* Description  : 저수위 감지 1시간 이상 유지시 저수위 미감지까지 배수
***********************************************************************************************************************/
void keep_1_hour_check(void)
{
    /* [2025-12-19] CH.PARK 저수위 감지 1시간 이상 유지시 저수위 미감지까지 배수 */
    if( u8DrainWaterLevel == DRAIN_LEVEL_LOW )
    {
        /*..hui [17-12-18오후 1:16:57] 드레인펌프 가동중에는 카운트 정지..*/
        if(Bit1_Drain_Low_1_Hour == SET)
        {
        ...
        if(gu16_drain_low_water_1min_tmr >= 600)
        {
            ...
            gu16_drain_low_water_1hour_tmr++;

            /*if(gu16_drain_low_water_1hour_tmr >= 180)*/
            // if(gu16_drain_low_water_1hour_tmr >= 120)
            if(gu16_drain_low_water_1hour_tmr >= 60)
            {
                Bit1_Drain_Low_1_Hour = SET;
                gu16_drain_low_water_1hour_tmr = 0;
            }
            else{}
        }
        else{}
...
void detect_drain_pump_error(void)
{
    ...
    /* [2025-12-19] CH.PARK 온수 추출 후 저수위 센서 감지 시 배수 수행 추가 */
    hot_water_extract_after_check();
    ...
    bit_drain_error_check_enable
        = ( ( Bit0_Drain_Level_Full | Bit1_Drain_Low_1_Hour
...
/***********************************************************************************************************************
* Function Name: hot_water_extract_after_check
* Description  : 온수 추출 후 저수위 센서 감지 시 배수를 수행하는 함수
***********************************************************************************************************************/
void hot_water_extract_after_check(void)
{
    if(F_Hot_Water_Extract_Completed == SET)
    {
        F_Hot_Water_Extract_Completed = CLEAR;
        if( u8DrainWaterLevel == DRAIN_LEVEL_LOW )
        {
            Bit11_Drain_Hot_Extract_After = SET;
        }
        else
        {
            if( u8DrainWaterLevel == DRAIN_LEVEL_EMPTY )
            {
                Bit11_Drain_Hot_Extract_After = CLEAR;
            }
            else{}
        }
    }
