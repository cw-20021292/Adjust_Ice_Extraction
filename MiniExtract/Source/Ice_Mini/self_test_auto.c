...
void check_cold_target_time(void)
{
    ...
    if( F_Comp_Output == SET )
    {
        ...
        // [2025-12-17] CH.PARK 컴프레셔 냉각 동작시간 제한 기존 60분에서 150분으로 변경 (상시진단 표준)
        if( gu16_cold_target_time_min >= SELF_TEST_COLD_ON_LIMIT_TIME )
        {
            gu16_cold_target_time_min = SELF_TEST_COLD_ON_LIMIT_TIME;
        }
        else{}

        if( gu8_Cold_Temperature_One_Degree <= gu8_cold_first_temp - 2  )       /* [냉수 현재온도 ≤ 초기온도 - 2℃]  → 냉각정상! 시간 기록 */
        {
        ...
        // else if( gu8_Cold_Temperature_One_Degree <= ( mu16_cold_off_temp / 10 ) ) /* [냉수 현재온도 ≤ 냉수OFF온도] → 냉각완료! 시간 기록 */
        else if( gu16_Cold_Temperature <= mu16_cold_off_temp )
        {
            /* [2015-12-17] CH.PARK 기존 반올림값으로 올리면 정상적으로 냉각완료가 되어도 WIFI로 컴프레셔 동작시간 데이터가 안올라가는 현상 개선
            * 기존 : gu8_Cold_Temperature_One_Degree 온도 반올림 변수값
            * 변경 : gu16_Cold_Temperature 실제 온도값
            */
            if( bit_cold_first_on_check == CLEAR )
            {
                bit_cold_first_on_check = SET;
