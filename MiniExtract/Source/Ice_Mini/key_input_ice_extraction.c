    ...
    if( u8IceOutState == ICE_SELECT__ICE_WATER )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /* 2025-10-02 CH.PARK 조각얼음은 추출할 때 정역회전 들어가면 안됨 */
                gu8_ice_feeder_shake = CLEAR;
                gu8_ice_feeder_shake_time = 0;

                /* 2025-10-02 CH.PARK 기존 30%에서 100% 닫는거로 변경 */
...
void ice_extraction_finish(void)
{
    if( F_IceOut == SET )
    {
        ...
        // [2025-12-29] CH.PARK V1.0.0.10부터 적용으로 임시 삭제
        #if 0
        /* 2025-10-28 CH.PARK 리드스위치 사용하는 모델인 경우에만 신규 사양적용 */
        if(model.u8model == MODEL_REED_USE)
        {
            /* 2025-10-28 CH.PARK 얼음걸림 체크 시작 */
            /* [2025-12-15] CH.PARK 얼음걸림 해제 10회 동작 시 얼름걸림 해제 동작 안함 */
