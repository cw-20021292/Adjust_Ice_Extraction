...
/***********************************************************************************************************************/
/* [2025-12-19] CH.PARK 온수 추출 완료 플래그 → 드레인탱크 배수에 사용 */
bit F_Hot_Water_Extract_Completed;
/***********************************************************************************************************************/
// [2025-01-14] 얼음 추출 시간 자동 조절 관련 함수 및 변수
bit bit_ice_full_old;
bit bit_first_icefull_check;
bit bit_first_icefull_check_finish;
U8  gu8_ice_out_timer;
U8  gu8_ice_make_count;
U8  gu8_ice_out_count;
U8  gu8_ice_time_step;
U8  gu8_ice_extract_time;
U8 gu8_ice_out_check_timer;
U32 gu32_ice_full_timer_hour;
static void up_ice_time(void);
static void down_ice_time(void);
static void decision_ice_out_time(void);
static void check_ice_extract_time(void);
/***********************************************************************************************************************
...
void start_effluent_water(void)
{
    ...
    switch( gu8_Water_Out_Step )
    {
        case STATE_0_STANDBY_STATE :
            ...
            && (F_firstEffluent_hotWater == CLEAR)
            && (gu8_Hot_Heater_Temperature_One_Degree >= 40)
            )
            {
                if(F_Airvent_TargetTempDown == CLEAR)
                {
                    F_Airvent_TargetTempDown = SET;
                }
            }
            else{}
        ...
        case STATE_21_HOT_PRE_HEATING_BUFFER_DRAIN_STATE :
            F_Airvent_TargetTempDown = CLEAR;
...
void stop_effluent_water(void)
{
    ...
    switch(gu8_Water_Out_Step)
    {
        ...
        case STATE_30_EXTRACT_VALVE_ON_STATE :
        case STATE_31_WATER_EXTRACT_STATE :
        // [2025-12-29] CH.PARK, 온수 정량 추출완료 후에는 드레인탱크 저수위 배수안되는 현상 개선 (드립커피)
        case STATE_32_WATER_AIR_VENT_STATE :
        case STATE_33_REMOVE_AIR_STATE :
        ...
        case STATE_40_EXTRACT_VALVE_OFF_STATE :
            ...
            if(gu8_effluent_timer >= VALVE_CONTROL_TIME_FOR_DEBUG)
            {
                ...
                if(u8WaterOutState == HOT_WATER_SELECT)
                {
                    F_Hot_Water_Extract_Completed = SET;
                }
            }
            else{}
...
void ice_extract_control(void)
{
    // [2025-01-14] CH.PARK, 얼음 추출 시간 자동 조절 관련 함수 (얼음3종 공통사양)
    check_ice_extract_time();
        ...
        case STATE_10_ICE_EXTRACT_STATE :
            if( F_IceBreak == CLEAR )
            {
                ...
                else
                {
                    ...
                    if((gu8_ice_extract_time >= ICE_EXTRACT_TIME__STEP_1_1)
                    && (gu8_ice_extract_time <= ICE_EXTRACT_TIME__STEP_4_4)
                    )
                    {
                        ice_extract_timer = gu8_ice_extract_time;
                    }
                    else
                    {
                        ice_extract_timer = ICE_EXTRACT_TIME__STEP_1_1;
                    }
                }
            }
            else
...
void stop_effluent_dripcoffee(void)
{
	...
	switch(gu8_Water_Out_Step)
	{
			...
			case STATE_30_EXTRACT_VALVE_ON_STATE :
			case STATE_31_WATER_EXTRACT_STATE :
            // [2025-12-29] CH.PARK, 온수 정량 추출완료 후 배수안되는 현상 개선 (일반 온수 및 MY WATER)
            case STATE_32_WATER_AIR_VENT_STATE :
            case STATE_33_REMOVE_AIR_STATE :
...
static void up_ice_time(void)
{
    /*if( gu8_ice_time_step < 5 )*/
    /*if( gu8_ice_time_step < 7 )*/
    if( gu8_ice_time_step < 6 )
    {
        gu8_ice_time_step++;
    }
    else{}
...
static void check_ice_extract_time(void)
{
    ...
    {
        bit_ice_full_old = CLEAR;
        gu8_ice_out_timer = 0;
        gu32_ice_full_timer_hour = 0;
        gu8_ice_make_count = 0;
        gu8_ice_out_count = 0;
        gu8_ice_time_step = 0;
        gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_1_1;
        bit_first_icefull_check_finish = CLEAR;

        return;
    }

    if( bit_first_icefull_check_finish == CLEAR )
    {
        if( bit_first_icefull_check == SET )
        {
            if( F_IceFull == SET )
            {
                gu8_ice_time_step = 0;
            ...
            {
                gu8_ice_time_step = 1;
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_1_2;
            }

            bit_first_icefull_check_finish = SET;
        }
    }

    if( F_IceOutCCW == SET && gu8IceOutCCWInterval == 0 )
    {
        gu8_ice_out_check_timer++;

        if( gu8_ice_out_check_timer >= 25 )
        {
            gu32_ice_full_timer_hour = 0;

            if( gu8_ice_out_check_timer >= 100 )
            {
                gu8_ice_out_check_timer = 100;
            }
            else{}
        }
        else{}
    ...
    else
    {
        //if( F_IR == CLEAR )
        //{
            /////if( gu8_ice_out_timer >= 15 && gu8_ice_out_timer < 25 )
            if( gu8_ice_out_check_timer >= 25 && gu8_ice_out_check_timer <= ICE_EXTRACT_TIME__STEP_1_4 )
            {
                gu8_ice_out_count++;

                if( gu8_ice_out_count >= ICE_OUT_COUNT )
                {
                    gu8_ice_out_count = ICE_OUT_COUNT;
                    up_ice_time();
                }
                else
                {
            ...
            /////else if( gu8_ice_out_timer >= 25 && gu8_ice_out_timer < 35 )
            else if( gu8_ice_out_check_timer >= ICE_EXTRACT_TIME__STEP_2_1 && gu8_ice_out_check_timer <= ICE_EXTRACT_TIME__STEP_2_4 )
            {
                gu8_ice_out_count++;
            ...
            else if( gu8_ice_out_check_timer >= ICE_EXTRACT_TIME__STEP_3_1 && gu8_ice_out_check_timer <= ICE_EXTRACT_TIME__STEP_3_4 )
            {

                up_ice_time();
            }
            //else if( gu8_ice_out_timer >= 50 )
            else if( gu8_ice_out_check_timer >= ICE_EXTRACT_TIME__STEP_4_1 )
            {
                up_ice_time();
                up_ice_time();
            }
            ...
            gu8_ice_out_check_timer = 0;
    }

    if( F_IceOutCCW == CLEAR )
    {
        gu32_ice_full_timer_hour++;

        /*if( gu32_ice_full_timer_hour >= ICE_FULL_TIME__2_HOUR )*/
        if( gu32_ice_full_timer_hour >= ICE_FULL_TIME__3_HOUR )
        {
            gu32_ice_full_timer_hour = 0;
            up_ice_time();
            gu8_ice_out_count = 0;
    ...
    if( gu8IceStep == STATE_41_GAS_SWITCH_HOT_GAS )
    {
        gu8_ice_make_count++;

        if( gu8_ice_make_count >= 3 )
        {
            gu8_ice_make_count = 0;
            down_ice_time();
            gu8_ice_out_count = 0;
    ...
    // 최종 추출 단계별 추출 시간 결정
    decision_ice_out_time();
}

/**
 * @brief 최종 추출 단계별 추출 시간 결정
 *
 */
static void decision_ice_out_time(void)
{
    if(u8IceOutState != ICE_SELECT__NONE)
    {
        if(gu8_ice_amount_step == ICE_LEVEL_1_STEP)
        {
            ...
            else if( gu8_ice_time_step == 1 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_1_2;
            }
            else if( gu8_ice_time_step == 2 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_1_3;
            }
            else /*if( gu8_ice_time_step == 3 )*/
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_1_4;
            }
        }
        else if(gu8_ice_amount_step == ICE_LEVEL_2_STEP)
        {
            if( gu8_ice_time_step == 0 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_2_1;
            }
            else if( gu8_ice_time_step == 1 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_2_2;
            }
            else if( gu8_ice_time_step == 2 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_2_3;
            }
            else /*if( gu8_ice_time_step == 3 )*/
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_2_4;
            }
        }
        else if(gu8_ice_amount_step == ICE_LEVEL_3_STEP)
        {
            if( gu8_ice_time_step == 0 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_3_1;
            }
            else if( gu8_ice_time_step == 1 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_3_2;
            }
            else if( gu8_ice_time_step == 2 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_3_3;
            }
            else /*if( gu8_ice_time_step == 3 )*/
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_3_4;
            }
        }
        else if(gu8_ice_amount_step == ICE_LEVEL_4_STEP)
        {
            if( gu8_ice_time_step == 0 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_4_1;
            }
            else if( gu8_ice_time_step == 1 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_4_2;
            }
            else if( gu8_ice_time_step == 2 )
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_4_3;
            }
            else /*if( gu8_ice_time_step == 3 )*/
            {
                gu8_ice_extract_time = ICE_EXTRACT_TIME__STEP_4_4;
